...
**The failure mechanism is exact.** `BasePlayerWeapon.Clip1` compiles to
...
fields, so its binding's `CanonicalPaths` is empty; the reference reader bounds-checks the
ordinal and reports absent
(`src/CS2OpenDev.Sdk.Entities.Abstractions/DictionaryEntityReader.cs:195-199`). If `AK47`
inherited `BasePlayerWeapon` today, every inherited property would silently read absent —
the issue describes this correctly.

**The invariant is real and verified in source, not taken from the issue.**
`LensClassState.Fields` is a `SortedDictionary` keyed `StringComparer.Ordinal`
(`src/CS2OpenDev.Sdk.Generator/SchemaLens/LensModels.cs:110`). The planning walk assigns
ordinals in that iteration order (`EntityWrapperEmitter.cs:124-147`), and
`EmitBinding` writes `CanonicalPaths` from the same `FieldPlan` list in the same order
(`EntityWrapperEmitter.cs:437-444`). Both are emitted from the one in-memory state in one
exporter pass — "two projections of one walk"
(`src/CS2OpenDev.Sdk.Exporter/Program.cs:426-428`). So the invariant has two parts, and
they are not equally load-bearing:

- *agreement by construction* between a wrapper's `Ord` constants and its manifest's
  `CanonicalPaths` — this is what correctness rests on, and it must survive;
- *ordinal-sortedness* of each class's path array — this is a repo convention. It appears
  in the emitter's header comment (`EntityWrapperEmitter.cs:21-26`) and in one test
  (`test/CS2OpenDev.Sdk.Entities.Tests/EmittedWrapperTests.cs:43-52`). It appears
  **nowhere in the contract**: `IEntityFieldReader` requires only that ordinal `i` is the
  field at `CanonicalPaths[i]`
  (`src/CS2OpenDev.Sdk.Entities.Abstractions/IEntityFieldReader.cs:9-15`),
  `EntityClassBinding` requires "dense from zero"
  (`EntityClassBinding.cs:25-31`), and `BindingConformance` checks density, duplicates,
  alias resolution and handle-ordinal range — not order
  (`BindingConformance.cs:48-99`). The distinction is the whole design space of this
  issue.
...
**The hierarchy, computed from the schema rather than quoted.** Walking each curated
class's `Parents[0]` chain in
...
`hasCuratedDescendant` already computes (`EntityWrapperEmitter.cs:81-94`). Uncurated
...
prefix, then own fields ordinal-sorted after it" — single-inheritance object layout
applied to ordinal spaces — which makes every base ordinal constant valid in every
...
- **Agreement by construction survives** — one layout computation per class, `Ord`
  constants and `CanonicalPaths` both projected from it, same walk, same pass.
- **Read path: unchanged.** Compile-time constant into an array-backed binding. No string
  lookup, no virtual call, no indirection, no per-instance state.
- **Contract: untouched.** See section 4.
- **Renumbering blast radius grows.** Today a rename renumbers one class's space; under
  ...
  cost.
- **Manifest size grows.** Total `CanonicalPaths` entries go 144 → 666 (sum of
  own + inherited across 58 classes) — 522 more path strings plus the inherited alias
  rows in `EntityWrapperRegistry.cs`. Tens of KB of generated data. The 45 markers'
  bindings stop being empty, which is the point: a marker's binding *is* its base's
  layout.
- **One new curation law, gated.** The same canonical path (or the same
  `targetProperty`) curated at two levels of one curated chain is now an error — today
  it is legal because every class owns its space. Checked against current state: no such
  collision exists. The emitter must fail it at generation with an error-severity
  diagnostic (the exporter exits non-zero on those — the post-3.0.7 guard), rather than
  letting `BindingConformance`'s duplicate check catch it at some consumer's startup.
...
(`IEntityFieldReader.cs:146`): boxed, engine-spelling, and documented "Not for hot paths"
(`IEntityFieldReader.cs:143`). Routing 144 typed properties through it costs a dictionary
hash per read, boxes every value type, and loses the typed accessors the dispatch table
exists to pick (`EntityWrapperEmitter.cs:172-201`). Doing it properly means adding typed
by-path members to `IEntityFieldReader` — an interface member addition, which breaks
every implementer of a frozen contract with an external implementation already validated
against it (`Abstractions/version.json`, the 0.3 note). That is the expensive contract
change the issue warns about, spent to buy a slower read path. It also quietly inverts
the design premise: the ordinal space exists precisely so reads are constant-time array
math; a contract whose base properties hash strings has conceded that premise.
...
shadowing is not ugly, it is **wrong**. Through a base-typed reference — which is exactly
what the restored `BasePlayerWeapon? ActiveWeapon` companion hands out — a `new`-shadowed
property executes the *base* body with the *base* ordinal against the *derived* binding,
and under sorted-merge layout silently reads the wrong field or absent. Trading "silently
null" for "silently wrong value" is a regression. The correct variant is
virtual-plus-override, which works but costs: a virtual dispatch on every read; 522
override bodies (666 − 144 — a 4.6× property surface, re-emitted on every regen); and it
*still* requires derived bindings to carry the inherited paths, because the derived
ordinals must address them. So option 3 = option 1's manifest growth, plus the emitted
bloat, plus a per-read vcall. Strictly dominated by prefix layout on every axis.

### why not option 4 — per-instance ordinal offset / indirection table

The base body becomes `Reader.TryReadInt32(_map[Ord.Clip1], ...)`. The map must reach the
wrapper at construction, so either `EntityWrapper`'s constructor changes — a contract
move: `Abstractions` ships `EntityWrapper` (`EntityWrapper.cs:25`), and a new constructor
parameter is source-breaking for every subclasser, including DemoViewer.NET's conformance
fixtures — or each generated class carries generated map data privately, adding an array
load to every read and per-instance state to a wrapper that is currently two references.
And the payoff is nil: under single inheritance, prefix layout **is** the offset table
with every offset zero. Option 4 is what you build when prefix layout is impossible —
curated multiple inheritance, or an immovable sorted-order requirement. Neither holds:
the second parents on curated chains are mixin interfaces the emitter already ignores,
and sortedness is a repo test, not a promise.
...
`BindingConformance.cs:30-101` — density, duplicates, alias resolution, handle range;
...
- 6 classes lose `sealed` and 51 gain a base type — additive;
...
4. **`test/CS2OpenDev.Sdk.Entities.Tests/EmittedWrapperTests.cs`.** Replace
   ...
   181 disappears. Add: derived projectile bindings carry the inherited origin alias and
   still conform; marker bindings equal their base's paths; and the pinned hierarchy
...
- that a registry-faithful runtime's storage materializes **inherited** fields for
  concrete weapon entities — that binding `AK47`'s manifest against a real flattened
