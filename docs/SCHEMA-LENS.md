...
PascalCase, then the word splitter. The derived name therefore obeys the
...
The grammar is one record per line, `<path> = <json-value>`, LF separators, LF
after the last line:
...
A `rename` op is an empirical claim that can be false. The script only drafts
it; whether the claim holds is on whoever commits the migration.
...
Non-.NET consumers need no package: fetch the file raw from
