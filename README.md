...
> **Upgrading?** Eight releases carry migration guides listing every affected name:
>
> - **[0.10](docs/MIGRATION-0.10.md)**: two unrelated upstream changes in one build. Valve interposed `CCSCustomPlayerCamera` between `CCSPlayerCamera` and `C_BaseEntity`: `Pawn` moved to the new base and is still inherited, `Enabled` and `IsControllingAngles` are gone in favour of a `CameraMode` enum, and `SchemaNames.CCSPlayerCamera` goes with the fields that named it. Separately, four `ECstrike15UserMessages` members gained a `_CSGOLegacy` suffix at unchanged numeric values.
