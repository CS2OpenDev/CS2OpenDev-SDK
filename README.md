...
src/
  CS2OpenDev.Sdk/             — the generated SDK (4k+ files; what the NuGet ships)
    ...
    Events/                       — one record per `.gameevents` entry (289 events)
  ...
  CS2OpenDev.Sdk.GameEvents/  — decoder, registry, envelope
    Generated/                    — exporter output: 289 factories + the name registry
...
| `CS2_GEN_006` | Info | A run-together lowercase word `WordSplitter` could not segment, so it was emitted unsegmented (`Somenewcompound`). Add its parts to the vocabulary if it is a compound, or to `Atomic` if it is one word. Currently empty. |
