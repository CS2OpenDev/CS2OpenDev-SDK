...
| `CS2_GEN_015` | Warning | Upstream's own `atomicCategory` (schema 2.1+) calls an atomic a container — `ATOMIC_COLLECTION_OF_T` or `ATOMIC_TT` — and `TypeMapper` stubbed it anyway. Aggregated per bare template name with a field count, because the individual names are already in `CS2_GEN_003` at a volume nobody reads. It currently fires on a live defect rather than a hypothetical one: schema 2.0 made atomic names fully templated (`CUtlVector< CGlobalSymbol >`) while the classification sets are still keyed bare, so no templated atomic matches. Reported, not fixed — correcting it changes ~1,931 property types and is a major-version event. |
